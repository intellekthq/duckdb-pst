...
inline constexpr column_t DUCKDB_VIRTUAL_COLUMN_START =
    UINT64_C(9223372036854775808);
inline constexpr auto PST_VCOL_PARTITION_INDEX = DUCKDB_VIRTUAL_COLUMN_START;
inline constexpr auto PST_VCOL_PARTITION_INDEX_TYPE = LogicalType::UBIGINT;
...
inline const auto RECIPIENT_SCHEMA =
    LogicalType::STRUCT({RECIPIENT_CHILDREN(SCHEMA_CHILD)});
...
inline const auto ATTACHMENT_SCHEMA =
    LogicalType::STRUCT({ATTACHMENT_CHILDREN(SCHEMA_CHILD)});
...
inline const auto ONE_OFF_RECIPIENT_SCHEMA =
    LogicalType::STRUCT({ONE_OFF_RECIPIENT_CHILDREN(SCHEMA_CHILD)});
...
inline const auto PST_SCHEMA =
    LogicalType::STRUCT({PST_CHILDREN(SCHEMA_CHILD)});

/* Base IPM.Note / Message schema, the base type for all PST reads that are not
 * folders */

#define NOTE_CHILDREN(LT)                                                      \
  /* TODO: these are computed properties (see spec: 2.4.3.2 Mapping between    \
   * EntryID and NID) */                                                       \
...
enum class NoteProjection {
  PST_CHILDREN(SCHEMA_CHILD_NAME) NOTE_CHILDREN(SCHEMA_CHILD_NAME)
};

inline const auto NOTE_SCHEMA = LogicalType::STRUCT(
    {PST_CHILDREN(SCHEMA_CHILD) NOTE_CHILDREN(SCHEMA_CHILD)});
...
enum class ContactProjection {
  PST_CHILDREN(SCHEMA_CHILD_NAME) NOTE_CHILDREN(SCHEMA_CHILD_NAME)
      CONTACT_CHILDREN(SCHEMA_CHILD_NAME)
};

inline const auto CONTACT_SCHEMA = LogicalType::STRUCT({PST_CHILDREN(
    SCHEMA_CHILD) NOTE_CHILDREN(SCHEMA_CHILD) CONTACT_CHILDREN(SCHEMA_CHILD)});
...
enum class AppointmentProjection {
  PST_CHILDREN(SCHEMA_CHILD_NAME) NOTE_CHILDREN(SCHEMA_CHILD_NAME)
      APPOINTMENT_CHILDREN(SCHEMA_CHILD_NAME)
};

inline const auto APPOINTMENT_SCHEMA =
    LogicalType::STRUCT({PST_CHILDREN(SCHEMA_CHILD) NOTE_CHILDREN(SCHEMA_CHILD)
                             APPOINTMENT_CHILDREN(SCHEMA_CHILD)});
...
enum class StickyNoteProjection {
  PST_CHILDREN(SCHEMA_CHILD_NAME) NOTE_CHILDREN(SCHEMA_CHILD_NAME)
      STICKY_NOTE_CHILDREN(SCHEMA_CHILD_NAME)
};

inline const auto STICKY_NOTE_SCHEMA =
    LogicalType::STRUCT({PST_CHILDREN(SCHEMA_CHILD) NOTE_CHILDREN(SCHEMA_CHILD)
                             STICKY_NOTE_CHILDREN(SCHEMA_CHILD)});
...
enum class TaskProjection {
  PST_CHILDREN(SCHEMA_CHILD_NAME) NOTE_CHILDREN(SCHEMA_CHILD_NAME)
      TASK_CHILDREN(SCHEMA_CHILD_NAME)
};

inline const auto TASK_SCHEMA = LogicalType::STRUCT({PST_CHILDREN(
    SCHEMA_CHILD) NOTE_CHILDREN(SCHEMA_CHILD) TASK_CHILDREN(SCHEMA_CHILD)});
...
enum class DistributionListProjection {
  PST_CHILDREN(SCHEMA_CHILD_NAME) NOTE_CHILDREN(SCHEMA_CHILD_NAME)
      DLIST_CHILDREN(SCHEMA_CHILD_NAME)
};

inline const auto DLIST_SCHEMA = LogicalType::STRUCT({PST_CHILDREN(
    SCHEMA_CHILD) NOTE_CHILDREN(SCHEMA_CHILD) DLIST_CHILDREN(SCHEMA_CHILD)});
...
enum class FolderProjection {
  PST_CHILDREN(SCHEMA_CHILD_NAME) FOLDER_CHILDREN(SCHEMA_CHILD_NAME)
};
