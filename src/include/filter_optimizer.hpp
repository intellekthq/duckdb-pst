...
class PSTFilterOptimizer : public OptimizerExtension {
  ...
  CreateProjection(idx_t new_table_index, LogicalGet &pst_logical_get);
  ...
  CreateLogicalFilter(map<idx_t, unique_ptr<TableFilter>> &non_vcol_filters,
                      ...
                      LogicalProjection &pst_expanded_projection);
  static void RewriteEligiblePSTFilters(OptimizerExtensionInput &input,
                                        unique_ptr<LogicalOperator> &plan,
                                        map<idx_t, idx_t> &binding_map);
  static void UpdateTableBindings(map<idx_t, idx_t> &binding_map,
                                  unique_ptr<LogicalOperator> &plan);
  static void EnsureNonVirtualColumnFilters(OptimizerExtensionInput &input,
                                            unique_ptr<LogicalOperator> &plan);
